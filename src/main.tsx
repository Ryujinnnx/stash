...
async function bootStash(): Promise<void> {
  try {
    if (!rootElement) {
...
function renderBootFailure(rootElement: HTMLElement | null, error: unknown): void {
  ...
  target.innerHTML = `
    <main style="min-height:100vh;background:#080810;color:#f0f0ff;display:grid;place-items:center;padding:24px;font-family:DM Sans,Inter,system-ui,sans-serif;">
      <section style="max-width:560px;border:1px solid rgba(255,255,255,.08);background:#0d0d1a;border-radius:20px;padding:28px;box-shadow:0 16px 48px rgba(0,0,0,.45);">
        ...
        <p style="margin:0 0 16px;color:rgba(240,240,255,.62);font-size:14px;line-height:1.6;">The app loaded its HTML, but one production JavaScript module failed before React could render. Share this message from the page or DevTools Console.</p>
        <code style="display:block;white-space:pre-wrap;overflow-wrap:anywhere;color:rgba(240,240,255,.62);background:#080810;border:1px solid rgba(255,255,255,.06);border-radius:10px;padding:12px;font:12px/1.6 DM Mono,monospace;">${escapeHtml(message)}</code>
      </section>
    </main>`;
}
